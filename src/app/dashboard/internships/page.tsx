...
export default async function InternshipsPage() {
  ...
  return (
    <main className="min-h-screen px-5 py-8 sm:px-8 xl:px-10">
      <section className="relative overflow-hidden mm-hero-panel p-6 sm:p-10">
        <div className="relative z-[1]">
          <p className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-white/70">
          ...
          <div className="mt-6 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 max-w-2xl text-sm leading-7 text-white/65 sm:text-[15px]">
            ...
            <div className="grid min-w-[220px] gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <div className="rounded-2xl border border-white/10 bg-white/10 px-6 py-4 backdrop-blur">
                ...
                <p className="mt-2 text-4xl font-bold text-white">{internships.length}</p>
              </div>
              <div className="rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-600 px-6 py-4 shadow-lg shadow-violet-900/30">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/80">Apply from dashboard</p>
                <p className="mt-2 text-sm font-semibold leading-6 text-white/90">One-click apply with your profile</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {internships.length === 0 ? (
        <section className="mt-10 rounded-3xl border border-dashed border-zinc-300 bg-white p-12 text-center dark:border-white/10 dark:bg-white/5">
          <h2 className="text-3xl font-bold mm-heading">No internships published yet</h2>
