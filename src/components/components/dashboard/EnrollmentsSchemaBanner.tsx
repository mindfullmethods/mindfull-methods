...
import { ENROLLMENTS_TABLE_SQL } from "@/lib/enrollments-schema-sql";

export default function EnrollmentsSchemaBanner() {
  const [copied, setCopied] = useState(false);

  async function handleCopy() {
    await navigator.clipboard.writeText(ENROLLMENTS_TABLE_SQL);
    setCopied(true);
  ...
  return (
    <div className="mb-6 rounded-2xl border border-amber-300 bg-amber-50 p-5 dark:border-amber-400/30 dark:bg-amber-400/10">
      ...
      <p className="mt-2 text-sm leading-6 text-amber-800 dark:text-amber-200">
        The <code className="rounded bg-amber-100 px-1 dark:bg-amber-900/40">enrollments</code> table is missing.
        Copy the SQL below, open <strong>Supabase → SQL Editor</strong>, paste, click <strong>Run</strong>, then refresh
        this page.
      </p>
