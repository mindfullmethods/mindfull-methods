...
export default function BusinessLeadForm() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setStatus("sending");
    try {
      const message = `[Business enquiry] Company: ${form.company} · Team size: ${form.teamSize}\n\n${form.message}`;
      ...
      setStatus("sent");
      setForm({ name: "", email: "", phone: "", company: "", teamSize: "11–50", message: "" });
    } catch {
  ...
  return (
    <section id="talk-to-us" className="mm-landing-section px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
      <div className="mx-auto max-w-2xl">
        <div className="text-center">
          <p className="mm-landing-tag">Talk to us</p>
          <h2 className="mt-4 text-3xl font-bold tracking-tight text-white sm:text-4xl">
            Share your <span className="text-violet-300">requirements</span>
          </h2>
          <p className="mt-3 text-sm text-white/50">
            Get an execution plan, pricing, and timeline for your operations—usually within 24 hours.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="mm-landing-glass mt-10 space-y-4 rounded-2xl p-6 sm:p-8">
          {status === "sent" ? (
            <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-4 text-sm font-semibold text-emerald-200">
              Thanks — we&apos;ll reach out with a tailored proposal shortly.
            </div>
          ) : null}
          {status === "error" && error ? (
          ...
          <textarea
            ...
            rows={5}
            placeholder="Tell us about your operations, locations, and expected volumes *"
            value={form.message}
